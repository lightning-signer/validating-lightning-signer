...
use lightning_storage_server::{client::Auth as LssAuth, Value};
...
async fn main() {
    ...
    let auth = LssAuth::new_for_client(&secret_key, &server_pubkey);

    let client = Client::new(rpc_url, &pubkey, &server_pubkey, &auth.shared_secret).await.unwrap();
    ...
    let client_hmac = compute_shared_hmac(&auth.shared_secret, &[0x01], &kvs); // in signer
    let server_hmac = compute_shared_hmac(&auth.shared_secret, &[0x02], &kvs); // in signer
    ...
    assert_eq!(res.len(), 2);
    check_get_result(&auth, &nonce, res, received_hmac);
...
// Checked in signer
fn check_get_result(auth: &Auth, nonce: &[u8], res: Mutations, received_hmac: Vec<u8>) -> Vec<(String, Value)> {
    let kvs: Vec<_> = res.into_iter().map(|(k, (v, d))| (k, Value { version: v as i64, value: d })).collect();
