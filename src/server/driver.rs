...
impl Signer for MySigner {
    ...
    async fn sign_mutual_close_tx(
        ...
        let node_id = self.node_id(msg.node_id)?;
        let channel_id = self.channel_id(&msg.channel_nonce)?;
        log_info!(self, "ENTER sign_mutual_close_tx({}/{})",
                  node_id, channel_id);
        ...
        let sigvec =
            self.sign_mutual_close_tx(
                &node_id, &channel_id, &tx,
        ...
        log_info!(self, "REPLY sign_mutual_close_tx({}/{}) sig={}",
    ...
    async fn sign_commitment_tx(
        ...
        log_info!(self, "REPLY sign_commitment_tx({}/{}) sig={}",
                  node_id, channel_id, hex::encode(&sigvec));
        Ok(Response::new(reply))
    }
    ...
    async fn sign_delayed_payment_to_us(
        ...
        let node_id = self.node_id(msg.node_id)?;
        let channel_id = self.channel_id(&msg.channel_nonce)?;
        log_info!(self, "ENTER sign_delayed_payment_to_us({}/{})",
                  node_id, channel_id);
        let reqtx = msg.tx.ok_or_else(|| self.invalid_argument("missing tx"))?;
