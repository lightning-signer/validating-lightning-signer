...
fn fake_network_test() {
    ...
    let _chan_1 = create_announced_chan_between_nodes(&nodes, 0, 1, make_features(), make_features());
    let _chan_2 = create_announced_chan_between_nodes(&nodes, 1, 2, make_features(), make_features());
    let _chan_3 = create_announced_chan_between_nodes(&nodes, 2, 3, make_features(), make_features());

    // Rebalance the network a bit by relaying one payment through all the channels...
    send_payment(&nodes[0], &vec!(&nodes[1], &nodes[2], &nodes[3])[..], 8000000, 8_000_000);
}

pub fn create_node_cfgs_with_signer(node_count: usize, signer: &Arc<MySigner>) -> Vec<NodeCfg<LoopbackChannelSigner>> {
    ...
    for i in 0..node_count {
        ...
        let keys_manager = Arc::new(keys_interface) as Arc<KeysInterface<ChanKeySigner=LoopbackChannelSigner>>;
